...
- **漏洞扫描模块**：全新页面，支持快速/全端口/自定义端口扫描，操作系统识别，Banner抓取，CPE版本范围匹配
- **CVE 服务中心**：独立部署的 Docker 容器化 CVE 查询服务（192.168.9.72:18080），NVD 全量数据后台下载
- **nuclei 验证引擎集成**：可选下载（~60MB），下载后自动发探测包验证漏洞真实性，不下载则使用版本匹配
- **日志分析页重写**：支持网络设备/安全/Linux 三类日志，AI 模型选择，进度动画，Markdown 导出

### 🎨 设计优化

- 侧栏宽度 260px → 220px，更紧凑
- 仪表盘卡片蓝色顶边渐变条 + hover 上浮效果
- 设备状态 LED 脉冲动画（online/running）
- 导航活跃项蓝色左侧指示条
- 数据表格行 hover 蓝色左边框
- 页面切换 animate-in 动效
- 品牌 Logo 更换为 Cisco 风格路由器 SVG + 双行文字排版

### 🐛 修复

- 模板 Tab 切换导致页面重载动画跳动（key 改为仅 pathname）
- 代理环境导致 CVE API 请求 502（添加 .no_proxy()）
- HTTP Banner 抓取超时过短导致产品识别失败（等待 2000ms + 多次读取）
- nuclei 3.11 兼容性修复（`-json` → `-j`，模板路径 `cves` → `http/cves`）
- 模型测试结果简化显示（成功/失败）

### 🔧 技术

- Rust: zip crate 解压 + reqwest no_proxy + nuclei 子进程调用
- Python: Flask CVE 服务 + NVD API 分页全量下载（~12h）
- SQLite: cpe_cves 表 + 版本范围匹配函数
- 全端口扫描支持 1-65535，并发 TCP 连接扫描
...
- **AI 配置检查**：AI评判前自动检查是否有可用的 AI 模型配置，无配置时给出指引
- **macOS Universal 构建**：CI 支持 Universal Binary（Intel + Apple Silicon）

### 🛠 修复

- **编译警告清理**：Windows 条件导入修复
- **GUI 框架调研**：egui/iced/slint 三方对比文档
...
- **Linux OS 版本详细上报**：从 `/etc/os-release` 读取 `PRETTY_NAME`，修复 Linux 上只显示发行版名无具体版本号的问题
- **统计服务端 ip2region 缺失**：Dockerfile 添加 `ip2region_v4.xdb`，修复 IP 归属地解析不可用
- **设备状态合并**：将设备在线状态和 SSH 账号状态合并为一个统一的状态显示，简化用户界面
...
- **详细操作系统版本上报**：匿名统计上报发行版 + 版本号（如 `Ubuntu 24.04`），不含内核版本
...
- **详细操作系统版本上报**：匿名统计的 OS 信息从 `windows`/`linux` 升级为详细版本
...
- **TFTP Server**：工具箱新增 TFTP 文件传输服务，支持上传/下载，进度条显示百分比/速度/方向，低端口自动弹出 pkexec 认证
- **Syslog 接收器**：工具箱新增 Syslog UDP 接收器，实时展示设备日志，支持低端口绑定
- **设备 CSV 批量导入导出**：设备列表支持 CSV 格式导入导出，自动检测表头/分隔符，冲突检测（同名跳过、同类型同IP跳过）
- **批量录入增强**：支持中文表头 + 空格/Tab/逗号三种分隔符自动识别
- **模板批量操作**：模板列表新增复选框批量选择、批量删除；删除前预查设备引用，有引用时禁止删除
- **模板引用计数**：模板列表新增"引用数"列，显示被多少设备引用
- **全平台发布**：CI 支持 Windows (MSI) + macOS (DMG) + Linux (DEB) 三平台统一发布

### 🐛 Bug 修复

- **全局审计修复 15 项**：竞态条件、panic 处理、数据一致性等问题（详见代码审查）
- **CSV 分隔符检测**：改为统计引号外逗号数量，避免引号内逗号误判
- **TFTP 大文件传输**：块大小回退 512B（标准兼容），修复大文件传输中断
- **模板删除 FK 约束**：删除模板时检查设备引用，防止外键约束静默失败
- **TFTP 最后一块确认**：等待 ACK 确认后才结束传输，curl/tftp 客户端正常退出

### 🔧 优化

- **TFTP 性能**：HashMap 存储 Arc<Vec<u8>> 代替克隆整个文件，4096B 块 + u64 偏移
- **TFTP 进度条**：百分比/速度/方向/动画，大文件传输体验提升
- **低端口绑定**：pkexec 子进程 bind 端口 + SCM_RIGHTS 传 fd，无需 root 权限启动应用
- **Zabbix 检测移除**：由 TFTP Server + Syslog 接收器替代
...
- **traceroute 域名解析错误提示**：DNS 解析失败时不再无声无结果，前端显示具体错误（如"名称或服务未知"）
- **操作列图标化**：设备/模板/AI配置页的操作按钮改为图标+tooltip，列宽从 260px 缩减到 150px，节省 40% 空间
- **设备状态优化**：在线但 SSH 账号错误时隐藏绿色"在线"徽章，只显示红色错误徽章，避免误导

### 🐛 Bug 修复

- **路由跟踪无结果**：`traceroute` 退出码非零时读取 stderr 并返回错误信息，之前只打日志、前端无声无结果
- **DataTable 无效宽度**：修复多处 `width: "w-24"` 等 Tailwind class 被当 CSS 值传入的 bug，宽度从未生效
- **模板列重叠**：修复模板列内容溢出盖住"最后检测"列的问题

### 🔧 优化

- **全页面表格列宽防撑爆**：DevicesPage/TemplatesPage/InspectionPage/ReportManagementPage/SettingsPage
  - 长文本列 `width` + `maxWidth` 截断（名称、模板名、URL 等）
