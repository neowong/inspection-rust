...
const ALL_VENDORS = [...NETWORK_VENDORS, ...SERVER_VENDORS.filter(v => !NETWORK_VENDORS.includes(v)), ...DB_VENDORS.filter(v => !NETWORK_VENDORS.includes(v) && !SERVER_VENDORS.includes(v))];
        ...
        <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={form.device_type} onChange={(e) => {
                    ...
                    } else if (dt === "other" && !ALL_VENDORS.includes(form.vendor)) {
                      updated.vendor = "其它";
                    } else if (dt !== "server" && dt !== "database" && dt !== "other" && !NETWORK_VENDORS.includes(form.vendor)) {
                      updated.vendor = "H3C";
                    }
                ...
                <div>
                  ...
                  <Select value={form.vendor} onChange={(e) => {
                    ...
                    {(form.device_type === "server" ? SERVER_VENDORS
                      : form.device_type === "database" ? DB_VENDORS
                      : form.device_type === "other" ? ALL_VENDORS
                      : NETWORK_VENDORS
              ...
              <div className="grid grid-cols-[5fr_5fr_2fr] gap-3">
                ...
                <div>
                  ...
                  <Input type="password" value={passwordSet ? "••••••••" : form.ssh_password} className={shakeFields.has("ssh_password") ? "animate-shake" : ""} onFocus={() => { if (passwordSet) { setPasswordSet(false); setForm({ ...form, ssh_password: "" }); } }} onChange={(e) => setForm({ ...form, ssh_password: e.target.value })} placeholder={editing ? "点击修改密码" : "输入密码"} />
                </div>
                <div>
