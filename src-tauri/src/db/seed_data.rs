...
pub fn seed_command_pool(conn: &mut Connection) -> Result<usize, String> {
    ...
    let commands: Vec<(&str, &str, &str, &str, i64)> = vec![
        ...
        ("MySQL", "mysql -e 'SHOW VARIABLES LIKE \"innodb_buffer_pool_size\"'", "InnoDB 缓冲池大小", "memory", 0),
        ("MySQL", "mysql -e \"SELECT * FROM sys.memory_by_host_by_current_bytes WHERE host != 'background'\"", "按主机查看内存使用", "memory", 0),
        ...
        ("PostgreSQL", "psql -c \"SELECT name, setting, unit FROM pg_settings WHERE name IN ('shared_buffers','effective_cache_size','work_mem','maintenance_work_mem','wal_buffers')\"", "内存相关配置", "memory", 0),
        ...
        ("Oracle", "echo \"SELECT NAME, ROUND(BYTES/1024/1024,1) AS MB FROM V\\$SGASTAT WHERE POOL='shared pool' AND NAME IN ('free memory','library cache','row cache') ORDER BY NAME;\" | sqlplus -S / as sysdba", "共享池使用情况", "memory", 0),
        ("Oracle", "echo \"SELECT COMPONENT, ROUND(CURRENT_SIZE/1024/1024,1) AS MB FROM V\\$SGA_DYNAMIC_COMPONENTS WHERE CURRENT_SIZE>0 ORDER BY CURRENT_SIZE DESC;\" | sqlplus -S / as sysdba", "SGA 各组件大小", "memory", 0),
        ("Oracle", "echo \"SELECT ROUND(SUM(BYTES)/1024/1024/1024,1) AS GB FROM V\\$SGAINFO WHERE NAME IN ('Fixed SGA Size','Buffer Cache Size','Shared Pool Size','Large Pool Size','Java Pool Size');\" | sqlplus -S / as sysdba", "SGA 总览", "memory", 0),
        ("Oracle", "echo \"SELECT NAME, ROUND(BYTES/1024/1024,1) AS MB FROM V\\$PGASTAT WHERE NAME IN ('total PGA allocated','total PGA inuse');\" | sqlplus -S / as sysdba", "PGA 使用情况", "memory", 0),
        ...
        ("SQL Server", "sqlcmd -Q \"SELECT name, value FROM sys.configurations WHERE name IN ('max server memory (MB)','min server memory (MB)','max degree of parallelism')\" -W", "关键配置参数", "memory", 0),
